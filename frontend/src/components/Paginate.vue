...
export default {
  ...
  computed: {
    isFirst() { return this.$props.current === 0; },
    isLast() { return (this.$props.current + this.$props.limit) >= this.$props.total; },
    page() { return Math.ceil(this.$props.current / this.$props.total); },
    pages() { return Math.ceil(this.$props.total / this.$props.limit); },
    buttons() {
      const buttons = [];
  ...
  methods: {
    goto(offset) { this.$emit('goto', offset); },
    prev(event) {
